...
const ExploreButton: React.FC<ExploreButtonProps> = ({
    ...
    return (
        <button type={type} onClick={handleClick} disabled={disabled}
            className={`group relative flex cursor-pointer justify-center gap-2 items-center mx-auto shadow-xl text-lg bg-gray-50 backdrop-blur-md lg:font-semibold px-6 py-2 overflow-hidden border-2 border-gray-50 rounded-full transition-all duration-300 hover:text-gray-50 disabled:opacity-50 disabled:cursor-not-allowed whitespace-nowrap ${className}`}>
            ...
            <p className="relative z-10 text-sm text-BlueDark group-hover:text-white">Consultoria grátis</p>
